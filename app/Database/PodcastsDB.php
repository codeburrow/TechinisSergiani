...
class PodcastsDB extends DB implements PostDbInterface
{
    ...
    {
        $stmt = $this->conn->prepare("SELECT * FROM kourtis.podcastsPosts WHERE urlName LIKE :urlName ORDER BY showDate DESC;");
    ...
    {
        try {
            $stmt = $this->conn->prepare("INSERT INTO kourtis.podcastsPosts (`title`, `summary`, `urlName`, `body`, `nameOfPhoto`)
